...
def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None):
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        score_id=round(float(scores[i]),3)
        score_text='{}'.format(score_id)
        # print(w*h)
        wh_text='{}'.format(int(w*h))
        wh_radio='{}'.format(round(float(w/h),3))
        if ids2 is not None:
            id_text = id_text + ', {}'.format(int(ids2[i]))
        _line_thickness = 1 if obj_id <= 0 else line_thickness
        color = get_color(abs(obj_id))
        cv2.rectangle(im, intbox[0:2], intbox[2:4], color=color, thickness=line_thickness)
        cv2.putText(im, id_text, (intbox[0], intbox[1] + 10), cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 0, 255),
                    thickness=2)
        cv2.putText(im, score_text, (intbox[0], intbox[1] +25), cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 0, 255),
                    thickness=2)
        cv2.putText(im, wh_text, (intbox[0], intbox[1] +40), cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 0, 255),
                    thickness=2)
        cv2.putText(im, wh_radio, (intbox[0], intbox[1] +55), cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 0, 255),
                    thickness=2)
    return im
